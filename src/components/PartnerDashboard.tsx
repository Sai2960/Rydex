...
function PartnerDashboard() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      console.log("videoKycStatus:", data.videoKycStatus);
      console.log("videoKycRoomId:", data.videoKycRoomId);
      console.log("activeStep will be:", data.partnerOnBoardingSteps + 1);
      setUserData(data);
      setActiveStep(data.partnerOnBoardingSteps + 1);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  // Poll every 5 seconds only when waiting for Video KYC
  useEffect(() => {
    if (activeStep !== 5) return;
    if (userData?.videoKycStatus === "in_progress" && userData?.videoKycRoomId)
      return;
  ...
  }, [activeStep, userData?.videoKycStatus, userData?.videoKycRoomId]);
      ...
      <div className="max-w-7xl mx-auto space-y-16">
          ...
          <>
            {userData?.videoKycStatus === "approved" ? (
              ...
              <ActionCard
                ...
                button="Join Call"
                onClick={() =>
                  router.push(`/partner/video-kyc/${userData.videoKycRoomId}`)
                }
              />
            ) : (
              <StatusCard
        ...
        {activeStep == 8 && vehicleData?.status == "approved" && (
          ...
          >
            ...
            <button className="mt-6 bg-white text-black px-6 py-3 rounded-xl font-semibold flex items-center gap-2 hover:bg-gray-200 transition">
