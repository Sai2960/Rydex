...
function page() {
  ...
  const [initialLoading, setInitialLoading] = useState(true); // ✅ separate state
  ...
  useEffect(() => {
    const handleGetVehicle = async () => {
      try {
        ...
        // 404 = no vehicle yet, silently ignore
        if (error?.response?.status !== 404) {
          console.error("Failed to load vehicle:", error);
        }
      } finally {
        setInitialLoading(false); // ✅ always clear initial load
      }
    };
    handleGetVehicle();
  }, []);
  ...
  if (initialLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <CircleDashed className="animate-spin text-gray-400" size={32} />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="mt-8 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {VEHICLES.map((v) => {
                const Icon = v.icon;
                const active = vehicleType === v.id; // ✅ use === not ==
                return (
                  <motion.div
                    key={v.id}
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          disabled={loading || !vehicleType || !vehicleNumber || !vehicleModel} // ✅ disable if incomplete
