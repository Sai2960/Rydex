...
function page() {
  ...
  const handleStartVideoKYC = async () => {
    try {
      const { data } = await axios.post(`/api/admin/video-kyc/start`, {
        partnerId: id,
      });
      router.push(`/partner/video-kyc/${data.roomId}`);
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 grid lg:grid-cols-3 gap-10">
        ...
        <div className="space-y-8">
          <AnimatedCard title={"Bank Details"} icon={<Landmark size={18} />}>
            ...
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">IFCS Code</span>
          ...
          {data?.partnerStatus == "pending" && (
            ...
            >
              ...
              <div className="flex flex-col gap-4">
                <button
                  className="py-3 rounded-2xl bg-linear-to-r from-black
                 to-gray-800 text-white font-semibold hover:opacity-90
                transition"
